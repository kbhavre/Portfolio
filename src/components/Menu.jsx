...
>
  ...
  {isMenuOpen && (
    <motion.div
      className="lg:hidden fixed top-0 left-0 w-full h-screen bg-secondary flex flex-col items-end justify-center px-4"
      initial={{ x: "-100%" }}
      animate={{ x: 0 }}
      exit={{ x: "-100%" }}
  ...
  <motion.div
    key={link.id}
    className="group text-[48px] font-raleway leading-tight font-medium text-primary relative"
    variants={menuVariants}
    initial="hidden"
    animate="visible"
    exit="hidden"
